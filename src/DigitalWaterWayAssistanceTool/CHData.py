...
def data_logic_proccs():
    ...
    for row in filtered_data: #filtered_data是筛选出rep1武汉局的数据
    ...
    # filename = 'class.json'
    # print(f"将filtered_data保存在 {filename}")
    # # 将rep写入到文件中
    # with open(filename, 'w', encoding='utf-8') as f:
    ...
    # print(filtered_data_by_area['010512'])
...
def ch_get_data():
    ...
    ch_result = [[], []]
    ch_result[0].append(i_excellent_data)
    ch_result[0].append(i_good_data)
    ch_result[0].append(i_average_data)
    ch_result[0].append(i_poor_data)
    ch_result[1].append(f_excellent_data)
    ch_result[1].append(f_good_data)
    ch_result[1].append(f_average_data)
    ch_result[1].append(f_poor_data)
