...
FROM (
    SELECT
        ...
        基础数据.水位站.站点名称,
				基础数据.水位站.序号,
        {小时数} AS 总数,
        {小时数} - COALESCE(t1.无数据数, 0) AS 有数据数,
        ROUND(({小时数}- COALESCE(t1.无数据数, 0)) / {小时数}* 100, 2) AS 完整率
    FROM
        基础数据.水位站
    LEFT JOIN (
        SELECT
        ...
        FROM
            水位数据整点缺失
				WHERE 水位数据整点缺失.缺失时间 >= '{查询开始时间}' AND 水位数据整点缺失.缺失时间 < '{查询结束时间}' 
        GROUP BY
            水位数据整点缺失.水位站
...
SELECT
    ...
    SUM(总数) AS 总数,
    SUM(有数据数) AS 有数据数,
    ROUND(SUM(有数据数) / SUM(总数) * 100, 2) AS 完整率
FROM (
    SELECT
        基础数据.水位站.站点名称,
        {小时数} AS 总数,
...
ORDER BY 序号;''')
    ...
    def 输出(sqlsession,check_date):
        结果 = sqlsession.query(水位完整率).filter(水位完整率.检查日期 == check_date).order_by(水位完整率.序号).all()
        return  结果

if __name__ == "__main__":
    检查结果_sqlsession, 检查结果_engine = sqlConfig.get_sql_session('检查结果')
    水位完整率.建表(水位完整率,检查结果_sqlsession, 检查结果_engine)
